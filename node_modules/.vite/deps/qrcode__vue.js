import { Ft as onMounted, Kn as ref, M as Fragment, U as computed, _n as watchEffect, cn as useId, nt as defineComponent, pt as h } from "./vue.runtime.esm-bundler-CTHQAePL.js";
//#region node_modules/qrcode.vue/dist/qrcode.vue.esm.js
/*!
* qrcode.vue v3.9.0
...
(function(qrcodegen) {
	qrcodegen.QrCode = function() {
		...
		QrCode.prototype.addEccAndInterleave = function(data) {
			...
			for (var i = 0; i < blocks[0].length; i++) _loop_1(i);
		...
		QrCode.prototype.getPenaltyScore = function() {
			...
			for (var x = 0; x < this.size; x++) {
				...
				for (var y = 0; y < this.size; y++) if (this.modules[y][x] == runColor) {
					runY++;
			...
			for (var y = 0; y < this.size - 1; y++) for (var x = 0; x < this.size - 1; x++) {
			...
			var dark = 0;
			for (var _i = 0, _a = this.modules; _i < _a.length; _i++) dark = _a[_i].reduce(function(sum, color) {
...
var _uid = 0;
function getUid() {
	if (typeof useId === "function") return "".concat(useId(), "-").concat(_uid++);
...
var DEFAULT_IMAGE_SIZE_RATIO = .1;
...
function generateRoundedPath(modules, margin, radius) {
	...
	var pathSegments = [];
	var r = Math.min(radius, .5);
	for (var row = 0; row < modules.length; row++) for (var col = 0; col < modules[row].length; col++) {
		...
		if (ne) pathSegments.push("A".concat(r, " ").concat(r, " 0 0 1 ").concat(x + 1, " ").concat(y + r));
		pathSegments.push("L".concat(x + 1, " ").concat(y + 1 - (se ? r : 0)));
		if (se) pathSegments.push("A".concat(r, " ").concat(r, " 0 0 1 ").concat(x + 1 - r, " ").concat(y + 1));
		pathSegments.push("L".concat(x + (sw ? r : 0), " ").concat(y + 1));
		if (sw) pathSegments.push("A".concat(r, " ").concat(r, " 0 0 1 ").concat(x, " ").concat(y + 1 - r));
		pathSegments.push("L".concat(x, " ").concat(y + (nw ? r : 0)));
		if (nw) pathSegments.push("A".concat(r, " ").concat(r, " 0 0 1 ").concat(x + r, " ").concat(y));
		pathSegments.push("z");
	}
...
function generatePath(modules, margin) {
	...
	for (var y = 0; y < modules.length; y++) {
		...
		for (var x = 0; x < row.length; x++) {
			...
			if (x === row.length - 1) {
				if (!cell) continue;
				if (start === null) pathSegments.push("M".concat(x + margin, ",").concat(y + margin, " h1v1H").concat(x + margin, "z"));
				else pathSegments.push("M".concat(start + margin, ",").concat(y + margin, " h").concat(x + 1 - start, "v1H").concat(start + margin, "z"));
				continue;
			}
...
function getImageSettings(cells, size, margin, imageSettings) {
	...
	return {
		x: imageX == null ? cells.length / 2 - w / 2 : imageX * scale,
		y: imageY == null ? cells.length / 2 - h / 2 : imageY * scale,
		h,
		w,
...
function useQRCode(props) {
	...
	var fgPath = computed(function() {
		if (props.radius > 0) return generateRoundedPath(cells.value, margin.value, props.radius);
	...
	return {
		...
		fgPath,
		imageProps,
		imageBorderProps: computed(function() {
...
var QrcodeSvg = defineComponent({
	...
	setup: function(props) {
		...
		var gradientVNode = computed(function() {
			...
			return h(props.gradientType === "linear" ? "linearGradient" : "radialGradient", __assign({ id: qrGradientId }, gradientProps), [h("stop", {
				offset: "0%",
		...
		var clipPathVNode = computed(function() {
			...
			return h("clipPath", { id: qrLogoClipPathId }, [h("rect", {
...
var QrcodeCanvas = defineComponent({
	...
	setup: function(props, ctx) {
		...
		var generate = function() {
			...
			if (gradient) {
				var grad = void 0;
				if (gradientType === "linear") grad = canvasCtx.createLinearGradient(0, 0, numCells.value, numCells.value);
				else grad = canvasCtx.createRadialGradient(numCells.value / 2, numCells.value / 2, 0, numCells.value / 2, numCells.value / 2, numCells.value / 2);
				grad.addColorStop(0, gradientStartColor);
				grad.addColorStop(1, gradientEndColor);
				canvasCtx.fillStyle = grad;
			} else canvasCtx.fillStyle = foreground;
			if (SUPPORTS_PATH2D) canvasCtx.fill(new Path2D(fgPath.value));
			else cells.value.forEach(function(row, rdx) {
				row.forEach(function(cell, cdx) {
					if (cell) canvasCtx.fillRect(cdx + margin.value, rdx + margin.value, 1, 1);
				});
			});
			if (props.imageSettings.src && image && image.naturalWidth !== 0 && image.naturalHeight !== 0 && imageProps.value) {
				...
				if (borderRadius > 0) {
					...
					canvasCtx.restore();
				} else canvasCtx.drawImage(image, imageProps.value.x, imageProps.value.y, imageProps.value.width, imageProps.value.height);
			}
		};
		onMounted(generate);
